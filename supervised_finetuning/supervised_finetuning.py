...
class ModelArguments:
    ...
    pad_token: Optional[str] = field(
        default=None,
        metadata={"help": "A special token used to make arrays of tokens the same size for batching purpose. "
...
class DataTrainingArguments:
    ...
    drop_out_of_length_limit_sample: bool = field(
        default=False,
        metadata={"help": "Drop out of length limit sample"},
    )
...
def load_tokenizer(tokenizer_class, data_args: DataTrainingArguments, model_args: ModelArguments, coordinator):
    ...
    if model_args.pad_token is not None:
        tokenizer_kwargs['pad_token'] = model_args.pad_token
...
def main():
    ...
    def drop_out_of_length_limit_sample(example):
        return not example['truncated']
...
#             train_dataset = train_dataset.sort(column_names=["seq_length"], reverse=True, keep_in_memory=False)
    ...
    if coordinator.is_master():
        print_special_token(tokenizer, model)
